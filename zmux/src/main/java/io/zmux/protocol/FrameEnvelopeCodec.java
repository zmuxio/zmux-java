...
public final class FrameEnvelopeCodec {
    ...
    private static final FrameByteReader<InputStream> INPUT_STREAM_FRAME_BYTE_READER =
            FrameEnvelopeCodec::readRequiredFrameByte;
    private static final FrameByteReader<FrameCodec.Decoder> DECODER_FRAME_BYTE_READER =
            FrameEnvelopeCodec::readRequiredFrameByte;

    private FrameEnvelopeCodec() {
    }

    public static FrameCodec.Frame readFrame(InputStream input, Limits limits) throws IOException {
    ...
    public static FrameCodec.Frame readFrame(FrameCodec.Decoder input, Limits limits) throws IOException {
    ...
    public static void writeFrame(OutputStream output, FrameCodec.Frame frame, Limits limits) throws IOException {
        byte[] payload = frame.payloadBytes();
        writeFrame(output, frame, null, payload, 0, payload.length, limits);
    }

    public static void writeFrame(OutputStream output,
    ...
    public static void writeFrame(GatheringByteChannel output, FrameCodec.Frame frame, Limits limits) throws IOException {
        byte[] payload = frame.payloadBytes();
        writeFrame(output, frame, null, payload, 0, payload.length, limits);
    }

    public static void writeFrame(GatheringByteChannel output,
                                  FrameCodec.Frame frame,
                                  byte[] payloadPrefix,
                                  byte[] payloadBytes,
                                  int payloadOffset,
    ...
    public static int gatherHeaderBytes(long frameLength, long streamId) throws IOException {
        return Varint62.length(frameLength) + 1 + Varint62.length(streamId);
    }

    public static int gatherBufferCount(byte[] payloadPrefix, int payloadLength) {
        int count = 1;
    ...
    public static int gatherBufferCount(byte[] payloadPrefix,
                                        byte[][] payloadParts,
                                        int payloadPartIndex,
                                        int payloadPartOffset,
                                        int payloadLength) {
    ...
    public static int gatherInlineBytes(FrameCodec.Frame frame, byte[] payloadPrefix, int payloadLength) {
    ...
    public static long appendFrame(GatherScratch scratch,
                                   FrameCodec.Frame frame,
                                   byte[] payloadPrefix,
                                   byte[] payloadBytes,
                                   int payloadOffset,
    ...
    public static int appendFrame(byte[] target,
                                  int offset,
                                  FrameCodec.Frame frame,
                                  byte[] payloadPrefix,
                                  byte[] payloadBytes,
    ...
    public static void writeGatheredBuffers(GatheringByteChannel output, GatherScratch scratch) throws IOException {
    ...
    public static long frameLength(long streamId, long encodedPayloadLength) throws IOException {
    ...
    private static Varint62.Decoded readStreamId(InputStream input, long frameLength) throws IOException {
        ...
        long value = readStreamIdValue(input, INPUT_STREAM_FRAME_BYTE_READER, first, length);
    ...
    private static Varint62.Decoded readStreamId(FrameCodec.Decoder input, long frameLength) throws IOException {
        int first = readRequiredFrameByte(input);
        int length = validatedStreamIdLength(first, frameLength);
        long value = readStreamIdValue(input, DECODER_FRAME_BYTE_READER, first, length);
        return decodedStreamId(value, length);
    }

    private static <T> long readStreamIdValue(T input,
                                              FrameByteReader<T> reader,
                                              int first,
        ...
        switch (length) {
            case 1:
                return first & 0x3fL;
            case 2:
                return ((first & 0x3fL) << 8) | reader.read(input);
            case 4:
                return ((first & 0x3fL) << 24)
                        | ((long) reader.read(input) << 16)
                        | ((long) reader.read(input) << 8)
                        | reader.read(input);
            case 8:
    ...
    private static int validatedStreamIdLength(int first, long frameLength) throws IOException {
        int length = 1 << (first >>> 6);
        if (frameLength < 1L + length) {
            throw FrameCodec.error(ErrorCode.FRAME_SIZE, "read frame", "frame too short");
        }
        return length;
    }

    private static void validateStreamIdCanonical(long value, int length) throws IOException {
        if (Varint62.length(value) != length) {
            throw FrameCodec.error(ErrorCode.PROTOCOL, "read frame", "non-canonical stream_id");
        }
    }

    private static int readRequiredFrameByte(InputStream input) throws IOException {
        int next = input.read();
        if (next < 0) {
            throw FrameCodec.error(ErrorCode.PROTOCOL, "read frame", "truncated frame");
        }
    ...
    public static final class GatherScratch {
        ...
        public void reset(int headerCapHint, int bufferCapHint) {
        ...
        public void ensureCapacity(int additionalHeaderBytes, int additionalBuffers) {
        ...
        public int reserveHeader(int length) {
    ...
    public static final class InboundFrame {
        ...
        public Runnable detachPayloadRelease() {
        ...
        public FrameCodec.Frame asFrame() {
